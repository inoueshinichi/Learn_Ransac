...
public:
    ...
    typename OptimFunc::params_type execute(const std::vector<T>& data)
    {
        ...
        for (int iter = 0; iter < max_iter_; ++iter) {
            ...
            auto params = this->estimate_params(selected_data);
            double error = 0.0;
            std::vector<T> inliers;
            for (int index = 0; index < data_size; ++index) {
                // 各データ点の値と推定値の誤差を計算
                error = this->calculate_error(params, data.at(index));
            ...
            if (inliers.size() > required_inliers_) {
                double model_error = this->calculate_model_error(params, data);
                this->set_candidate_model(model_error, params);
            }
        }

        // 正しいモデル候補から最小誤差に対応するモデルパラメータを選択
        auto optimized_params = this->select_optimized_params();
